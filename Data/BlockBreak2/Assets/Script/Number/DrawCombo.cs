...
public class DrawCombo : MonoBehaviour {
	...
	void Start () {
		...
		for (int Cnt = 0; Cnt < m_Digit.Length; Cnt++)
		{
			...
			m_Digit [Cnt].transform.position = new Vector3 (this.transform.position.x + (float)(Cnt * 0.3),this.transform.position.y,this.transform.position.z);
	...
	void Update () {
		...
		transform.position += new Vector3(0,m_AddPosY,0);
		m_AddPosY *= 0.9f;
